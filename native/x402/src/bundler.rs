...
pub async fn send_to_arweave(di: DataItem) -> Result<Vec<u8>, Error> {
    let bundler = BUNDLER_CLIENT
        .lock()
        .map_err(|_| anyhow!("error lazy initializing the bundle turbo client"))?;
    ...
    let di_bytes = di.to_bytes().map_err(|err| anyhow!("failed to serialize data item: {err}"));
    di_bytes
}
